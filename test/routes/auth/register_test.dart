...
void main() {
  ...
  const requestModel = CreateUserReq(name: '',email: 'email', password: 'password');
  ...
  setUp(() {
    ...
    when(() => uri.resolve(any())).thenAnswer(
      (_) => Uri.parse('http://localhost:8080/auth/register'),
    );
  ...
  group('POST /auth', () {
    ...
    test('POST /auth - Successful Register', () async {
      // Arrange
      when(() => request.method).thenReturn(HttpMethod.post);
      ...
      when(() => request.json()).thenAnswer(
        (_) async => {
          'name': 'name', // Replace with valid name
      ...
      when(() => authServiceImpl.registerUser(any()))
